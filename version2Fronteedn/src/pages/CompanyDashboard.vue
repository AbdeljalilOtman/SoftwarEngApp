<template>
  ...
  export default {
    ...
    async mounted() {
      try {
        const me = await api.get("/api/recruiters/me");
    ...
    methods: {
      async listMyJobs() {
        try {
          const res = await api.get("/api/recruiters/my-jobs");
          this.jobs = res.data || [];
        } catch (error) {
      ...
      viewApplicants(jobId) {
        alert(`View applicants for Job ID: ${jobId}`); // Replace with actual navigation or logic
      },
    },
  };
  </script>
