<template>
  <div class="create-job-post container-small">
    <div class="header-row">
      ...
      <router-link :to="`/company-dashboard/${recruiter}`" class="btn-back">Back to Dashboard</router-link>
    </div>
...
export default {
  ...
  mounted() {
    this.recruiter =  this.$route.params.id;
  },
  methods: {
    async submitJob() {
  ...
  try {
    const jobData = {
      ...
      location: this.location || null, // Handle optional fields
    ...
    const response = await axios.post(`http://localhost:8080/api/jobs/create/${this.recruiter}`,JSON.stringify(jobData));
    console.log("Job created:", response.data);
    alert("Job post created successfully!");
    //this.$router.push(`/company-dashboard/${this.companyId}`);
  } catch (error) {
