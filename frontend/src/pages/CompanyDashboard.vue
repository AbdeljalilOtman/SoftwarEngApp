<template>
    <div class="company-dashboard">
      ...
      <!-- Navigation Actions -->
      <div class="actions">
        <button class="btn-primary" @click="createNewJobPost">
  ...
  async mounted() {
    const recruiterId = this.$route.params.id;
    this.companyId = recruiterId; // Get the recruiter ID from the path parameter
    try {
      const me = await api.get(`/api/recruiter/${recruiterId}`);
      this.$store.commit("setMe", me.data);
      this.companyName = me.data.companyName || "Company";
      this.companyId = me.data.id;
    } catch (error) {
  ...
  methods: {
    createNewJobPost() {
    this.$router.push({ path: `/create-job/${this.companyId}` });
  },
    async listMyJobs(recruiterId) {
      try {
        const res = await api.get(`/api/recruiter/${recruiterId}/my-jobs`);
        this.jobs = res.data || [];
      } catch (error) {
