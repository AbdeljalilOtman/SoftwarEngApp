...
export default createStore({
  ...
  mutations: {
    ...
    setMe(state, me) {
      state.me = me; // Update me in the store
    },
  ...
  actions: {
    async registerUser({ commit }, payload) {
      try {
        ...
        const { data } = await api.post('/api/job-seekers/register', payload, { headers: { 'Content-Type': 'application/json' } });
    ...
    async registerCompany({ commit }, payload) {
      try {
        // Send the registration request to the backend
        const { data } = await api.post("/api/recruiter/register", payload);
